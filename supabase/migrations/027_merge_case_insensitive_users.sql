-- ============================================================
-- 027_merge_case_insensitive_users.sql
-- Auto-migrates and deduplicates user accounts, normalizes all emails,
-- and installs automatic lowercase triggers for all future operations.
-- ============================================================
  ...
  IF to_regclass('public.users') IS NOT NULL THEN
    FOR r IN (
      ...
      FOR i IN 2..array_length(r.user_ids, 1) LOOP
        ...
        IF to_regclass('public.esim_cash_accounts') IS NOT NULL THEN
          UPDATE public.esim_cash_accounts SET user_id = primary_id WHERE user_id = dup_id;
        END IF;
        
        IF to_regclass('public.esim_cash_transactions') IS NOT NULL THEN
...
BEGIN
  ...
  IF to_regclass('public.orders') IS NOT NULL THEN
    UPDATE public.orders SET customer_email = LOWER(TRIM(customer_email)) WHERE customer_email IS NOT NULL AND customer_email != LOWER(TRIM(customer_email));
  END IF;
  
  IF to_regclass('public.crypto_sessions') IS NOT NULL THEN
    UPDATE public.crypto_sessions SET customer_email = LOWER(TRIM(customer_email)) WHERE customer_email IS NOT NULL AND customer_email != LOWER(TRIM(customer_email));
  END IF;
  
  IF to_regclass('public.esim_cash_accounts') IS NOT NULL THEN
    UPDATE public.esim_cash_accounts SET email = LOWER(TRIM(email)) WHERE email IS NOT NULL AND email != LOWER(TRIM(email));
  END IF;
  
  IF to_regclass('public.esim_cash_transactions') IS NOT NULL THEN
    UPDATE public.esim_cash_transactions SET email = LOWER(TRIM(email)) WHERE email IS NOT NULL AND email != LOWER(TRIM(email));
  END IF;
  
  IF to_regclass('public.support_tickets') IS NOT NULL THEN
    UPDATE public.support_tickets SET customer_email = LOWER(TRIM(customer_email)) WHERE customer_email IS NOT NULL AND customer_email != LOWER(TRIM(customer_email));
  END IF;
  
  IF to_regclass('public.ticket_messages') IS NOT NULL THEN
    UPDATE public.ticket_messages SET sender_email = LOWER(TRIM(sender_email)) WHERE sender_email IS NOT NULL AND sender_email != LOWER(TRIM(sender_email));
  END IF;
  ...
  IF to_regclass('public.sent_emails') IS NOT NULL THEN
    UPDATE public.sent_emails SET recipient_email = LOWER(TRIM(recipient_email)) WHERE recipient_email IS NOT NULL AND recipient_email != LOWER(TRIM(recipient_email));
  END IF;
END $$;
...
DO $$
BEGIN
  IF to_regclass('public.esim_cash_accounts') IS NOT NULL AND to_regclass('public.users') IS NOT NULL THEN
...
BEGIN
  IF TG_OP = 'INSERT' OR TG_OP = 'UPDATE' THEN
    IF TG_TABLE_NAME IN ('orders', 'crypto_sessions', 'support_tickets') THEN
      ...
      END IF;
    ELSIF TG_TABLE_NAME IN ('ticket_messages', 'support_messages') THEN
...
-- Attach triggers safely only if tables exist
DO $$
BEGIN
  IF to_regclass('public.users') IS NOT NULL THEN
  ...
  IF to_regclass('public.ticket_messages') IS NOT NULL THEN
    DROP TRIGGER IF EXISTS trg_normalize_email_ticket_messages ON public.ticket_messages;
    CREATE TRIGGER trg_normalize_email_ticket_messages
